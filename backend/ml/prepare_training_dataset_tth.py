...
# ---- load FEATURES (symbols list) ----
FEATURES_PATH = os.getenv("TTH_FEATURES_PATH", "").strip()

candidates = []
if FEATURES_PATH:
    candidates.append(pathlib.Path(FEATURES_PATH))

# common defaults in XauTrendLab repo
candidates += [
    pathlib.Path("/opt/xauapi/ml/xgb_reg.json"),
    pathlib.Path("/opt/xauapi/ml/features.json"),
    pathlib.Path("/opt/xauapi/api/trend/xgb_reg.json"),
    pathlib.Path("/opt/xauapi/api/trend/features.json"),
]
...
if not isinstance(FEATURES, dict) or "symbols" not in FEATURES:
    raise RuntimeError(
        f"FEATURES not found or missing 'symbols'. "
        f"Set env TTH_FEATURES_PATH to a JSON file containing {{'symbols':[...]}}, "
        f"or place one of: {[str(x) for x in candidates]}"
    )
